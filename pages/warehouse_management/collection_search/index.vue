  ...
  <div class="body_style">
    <div class="content-box">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>藏品查询</el-breadcrumb-item>
      </el-breadcrumb>
       <el-divider></el-divider>
      <h2>藏品查询</h2>
      <div class="formstyle">
        <el-card class="waresourse_search_boxcard">
          <el-tabs v-model="activeName" @tab-click="differentStatus">
            <el-tab-pane label="藏品风险评估通过未申请入库查询" name="first">
            </el-tab-pane>
            <el-tab-pane label="藏品风险评估通过已申请入库查询" name="second">
            </el-tab-pane>
            <el-tab-pane label="藏品入库通过查询" name="third"> </el-tab-pane>
            <el-tab-pane label="藏品出库通过查询" name="fourth"> </el-tab-pane>
          </el-tabs>
          <!--查询表单  inline让表单在一行显示  -->
        ...
        >
          ...
          >
            ...
            <el-form-item  label="风险评估通过时间" v-if="activetab=='one'|| activetab=='two'">
              <el-input v-model="ColBasicForm.objectRiskpasstime"></el-input>
            </el-form-item>
            <el-form-item label="风险评估提交人" v-if="activetab=='one'">
              <el-input v-model="ColBasicForm.objectRiskPersonid"></el-input>
            </el-form-item>
            <el-form-item label="风险评估通过管理员" v-if="activetab=='one'|| activetab=='two'">
              <el-input v-model="ColBasicForm.adminRiskId"></el-input>
            </el-form-item>
            <el-form-item label="提交入库操作人员" v-if="activetab=='two'|| activetab=='three'">
              <el-input v-model="ColBasicForm.objectEnterPersonid"></el-input>
            </el-form-item>

            <el-form-item label="藏品入库时间" v-if="activetab=='three'">
              <el-input v-model="ColBasicForm.objectEntertime"></el-input>
            </el-form-item>
            <el-form-item label="入库通过管理员" v-if="activetab=='three'">
              <el-input v-model="ColBasicForm.adminEnterId"></el-input>
            </el-form-item>

            <el-form-item label="提交出库操作人员" v-if="activetab=='four'">
              <el-input v-model="ColBasicForm.objectOutPersonid"></el-input>
            </el-form-item>
            <el-form-item label="藏品出库时间" v-if="activetab=='four'">
              <el-input v-model="ColBasicForm.objectOuttime"></el-input>
            </el-form-item>
            <el-form-item label="出库通过管理员" v-if="activetab=='four'">
              <el-input v-model="ColBasicForm.adminOutId"></el-input>
            </el-form-item>
          </el-form>
...
export default {
  ...
  methods: {
    ...
    queryObjectByIf() {
      ...
      if (this.activeName == "first") {
        //藏品风险评估通过未申请入库查询
        wm.getInOutStatusByIds(2).then((response) => {
          let arr = [];
          //对象用of
      ...
      {
        wm.getInOutStatusByIds(4).then((response) => {
          let arr = [];
          //对象用of
          if (response.data != null) {
            for (let obj of response.data.data.StatusListId) {
      ...
      {
        wm.getInOutStatusByIds(6).then((response) => {
          let arr = [];
          //对象用of
          if (response.data != null) {
            for (let obj of response.data.data.StatusListId) {
    ...
    //切换tab时展示不同的查询结果入库 出库 风险评估通过
    differentStatus(tab, event) {
      if (tab.name == "first") this.RiskPassOne();
      if (tab.name == "second") this.RiskPassTwo();
      if (tab.name == "third") this.EnterPass();
      if (tab.name == "fourth") this.OutPass();
    },
    //藏品风险评估通过未申请入库查询 2
    RiskPassOne() {
      this.tableData = [];
    ...
    //获取藏品类别信息
    getAllClassInfo() {
      wm.getClassInfo().then((response) => {
        this.allObjectClass = response.data.data.classlist;
      });
    },
    //获取藏品残缺程度信息
    getAllFalutsInfo() {
      wm.getFaultsInfo().then((response) => {
...
.waresourse_search_boxcard .findtable {
  margin-top: 20px;
  margin-bottom: 30px;
}

.waresourse_search_boxcard .el-tabs__item {
  font-weight: 600;
  padding: 0 70px;
}

.waresourse_search_boxcard .demo-form-inline{
  width: 100%;
}
</style>