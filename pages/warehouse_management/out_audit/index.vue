...
export default {
  ...
  created(){
    this.getOutApplyColList();
  },
  methods: {
    //获取出库申请的藏品列表
    getOutApplyColList(){
      wm.getInOutStatusByIds(8).then(response=>{
        let arr=[];
    ...
    //根据藏品id获取对应出库申请的藏品基本信息
    getList(page=1){
      this.page=page;
      wm.getBasicInfoByIds(this.arraylist,this.page,this.limit).then(response=>{
    ...
    applypass(id){
      ...
      if (userStr) {
        //入库提交人员以及操作人员
        this.InOutStatusForm.adminOutId = JSON.parse(userStr).id;
        this.InOutStatusForm.peopleRecord = JSON.parse(userStr).id;
        this.InOutStatusForm.objectStatusId=id;
        this.InOutStatusForm.objectStatus=6;  //出库通过
        wm.updateOutAuditStatus(this.InOutStatusForm).then(response=>{
          if(response.data.code==200)
          {
             this.$message.success("出库审核通过");
          }
          else
          {
    ...
    applyreject(id){
      ...
      if (userStr) {
        ...
        this.InOutStatusForm.peopleRecord = JSON.parse(userStr).id;
        this.InOutStatusForm.objectStatusId=id;
        this.InOutStatusForm.objectStatus=7;
        wm.updateOutAuditStatus(this.InOutStatusForm).then(response=>{
          if(response.data.code==200){
            this.$message.success("藏品出库已驳回");
          }
          else
          {
            this.$message.error("操作失败");
